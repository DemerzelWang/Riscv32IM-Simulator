 ...
 */
 ...
 bool MemoryManager::setLong(uint32_t addr, uint32_t val, uint32_t *cycles) {
   if (!this->isAddrExist(addr)) {
 ...
 uint32_t MemoryManager::getLong(uint32_t addr, uint32_t *cycles) {
   uint32_t b1 = this->getByte(addr, cycles);
   uint32_t b2 = this->getByte(addr + 1);
   uint32_t b3 = this->getByte(addr + 2);
   uint32_t b4 = this->getByte(addr + 3);
   uint32_t b5 = this->getByte(addr + 4);
   uint32_t b6 = this->getByte(addr + 5);
   uint32_t b7 = this->getByte(addr + 6);
   uint32_t b8 = this->getByte(addr + 7);
